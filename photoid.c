...
int uat_subpacket_type(struct openpgp_packet *packet)
{
	...
	if (packet == NULL || packet->data == NULL || packet->length < 2) {
		return -1;
	}

	/* First subpacket length: 1, 2 or 5 octets — same encoding
	 * getphoto() parses above. */
	length = packet->data[j++];
	if (length < 192) {
		/* length is correct */
	} else if (length < 255) {
		if (packet->length < 3) {
	...
	/* The subpacket length covers the type octet, so a valid
	 * subpacket has length >= 1 and fits in the packet. */
	if (length < 1 || (packet->length - j) < length) {
		return -1;
	}

	return packet->data[j];
}
